---
title: User Guide
description: A user guide for OKDP control platform.
---
...
On the top left we have the choice between `Platform` and `Views`. Let's first look at `Platform` which contains the Services and the `Project Panel`.

![Dashboard](../../assets/dashboard.png)

### Services

The OKDP service catalog is on the left toolbar when clicking on the tab `Platform`. Services.
...
How to create or edit `Connections` is explained in the [Admin guide](../admin-guide/example.md). However, you can view the details by clicking on the line of the connection.

![Connection](../../assets/connection.png)

In the `Settings` section, the project description and color can be modified as well as the deletion of the project can be performed. Moreover, you have the option to create `Costum views`. Their role is to display a tab for a service which is outside of the OKDP platform service scope.
...
Views give access to the user interfaces of deployed services as well as non-OKDP added service through the `Custom views` option under the categories mentioned above.
...
How to instantiate and modify a service is explained in the [Admin guide](../admin-guide/example.md) to the exception of Airflow as the usage coincides with its deployment. Otherwise, we describe here, how to use an already deployed service.

Each time you sea the small window sign, it brings you to the user interface of the service.
...
Click on the window mentioned before, either in the View or in the service window shown at the beginning, and you will have the option between four images.
...
Choose a connection to a database. If it does not exist, it can be created by clicking `+ New connection`, however, the procedure to create it is explained in the [Admin guide](../admin-guide/example.md). Then configure the Dag with the Git syncing parameters.
...
You then have a recapitalution of the deployment. Click on `Deploy instance` to start the deployment.
